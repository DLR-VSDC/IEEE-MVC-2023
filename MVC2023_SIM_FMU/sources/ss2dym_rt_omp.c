...
#define Matlab6
#endif
#if !defined(LABCAR) && !defined(xMOD)
#define NO_FILE
#endif
...
#endif
DYMOLA_STATIC int DYNEventInUpdate_;
...
#if GenerateResultInNonDymosim
static const char_T* declPhaseErr = "Out of memory when generating result file from DymolaBlock\n";
#endif
...
#if GenerateResultInNonDymosim
	  if (DeclarePhase9(&phase)) {
		  ssSetErrorStatus(S, declPhaseErr);
		  return;
	  }
	  if (DeclarePhase0(cuser_)) {
		  ssSetErrorStatus(S, declPhaseErr);
		  return;
	  };
#endif
...
#endif	
#if GenerateResultInNonDymosim
	if (DeclarePhase1(&phase)) {
		ssSetErrorStatus(S, declPhaseErr);
		return;
	}
	if (DeclarePhase3(&phase, *time, ssGetTFinal(S), dp_, x_, xd_, y_, u_, w_)) {
		ssSetErrorStatus(S, declPhaseErr);
		return;
	}
#endif
...
#endif
#if GenerateResultInNonDymosim
	  if (DeclarePhase7(&phase, time, 10, dp, x, &duser[0], &duser[nx], u, &duser[nx + ny])) {
		  ssSetErrorStatus(S, declPhaseErr);
		  return;
	  }
#endif
