...
#define startstringmark  (DYN_GetThreadData()->m_extra.m_startstringmark)
#define EndRealbuffer (DYN_GetThreadData()->m_extra.m_endreal)
#define EndIntegerbuffer (DYN_GetThreadData()->m_extra.m_endinteger)
#define EndSizebuffer (DYN_GetThreadData()->m_extra.m_endsize)
#define EndStringbuffer (DYN_GetThreadData()->m_extra.m_endstring)
#define Endsimplestring (DYN_GetThreadData()->m_extra.m_endsimplestring)
DYMOLA_STATIC void EnsureMarkInitializedFunction() {
	struct DYN_ThreadData*threadData=0;
	threadData=DYN_GetThreadData();
	if (threadData && 0==threadData->m_extra.m_endreal) {
		const int numReal=
	...
	;
		...
		threadData->m_extra.m_endreal=threadData->m_current.place.Realbuffer+numReal;
		threadData->m_current.place.Integerbuffer=(Integer*)calloc(numInteger,sizeof(Integer));
		threadData->m_extra.m_endinteger=threadData->m_current.place.Integerbuffer+ numInteger;
		threadData->m_current.place.Sizebuffer=(SizeType*)calloc(numInteger,sizeof(SizeType));
		threadData->m_extra.m_endsize=threadData->m_current.place.Sizebuffer+ numInteger;
		threadData->m_current.place.Stringbuffer=(String*)calloc(numString,sizeof(String));
		threadData->m_extra.m_endstring=threadData->m_current.place.Stringbuffer+ numString;
		threadData->m_current.mark=threadData->m_current.place;
		threadData->m_start=threadData->m_current;
		threadData->m_stringmark=threadData->m_extra.m_startstringmark=calloc(numString,1);
		threadData->m_extra.m_endsimplestring=threadData->m_stringmark+ numString;
	}
}
DYMOLA_STATIC void EnsureMarkFree(struct DYN_ThreadData*threadData) {
	if (threadData && 0!=threadData->m_extra.m_endreal) {
		...
		free((void*)threadData->m_extra.m_startstringmark);

		threadData->m_start.place.Realbuffer=0;
		threadData->m_extra.m_endreal=0;
		threadData->m_start.place.Integerbuffer=0;
		threadData->m_extra.m_endinteger=0;
		threadData->m_start.place.Sizebuffer=0;
		threadData->m_extra.m_endsize=0;
		threadData->m_start.place.Stringbuffer=0;
		threadData->m_extra.m_endstring=0;
		threadData->m_start.mark=threadData->m_start.place;
		threadData->m_current=threadData->m_start;
		threadData->m_stringmark=threadData->m_extra.m_startstringmark=0;
		threadData->m_extra.m_endsimplestring=0;
	}
}
...
DYMOLA_STATIC Integer *DYNJIntegerTemp(SizeType r, struct DYNFunctionData_*DYNStackData_) {
	...
	Assert(DYNStackData_->mcurrentMark->place.Integerbuffer<DYNStackData_->m_extra.m_endinteger, "Out of memory for integers\nIt could due to too large matrices, infinite recursion, or uninitialized variables.\nYou can increase the size of 'Integerbuffer' in dymola/source/matrixop.h.");
	return i;
}
DYMOLA_STATIC SizeType *DYNJSizeTemp(SizeType r, struct DYNFunctionData_*DYNStackData_) {
	...
	Assert(DYNStackData_->mcurrentMark->place.Sizebuffer<DYNStackData_->m_extra.m_endsize, "Out of memory for array dimensions\nIt could due to too many matrices, infinite recursion, or uninitialized variables.\nYou can increase the size of 'Sizebuffer' in dymola/source/matrixop.h.");
	return s;
}
DYMOLA_STATIC String * DYNJStringTemp(SizeType r, struct DYNFunctionData_*DYNStackData_) {
	String *s = 0;
	s = DYNStackData_->mcurrentMark->place.Stringbuffer;
	DYNStackData_->mcurrentMark->place.Stringbuffer += r;
	Assert(DYNStackData_->mcurrentMark->place.Stringbuffer<DYNStackData_->m_extra.m_endstring, "Out of memory for strings\nIt could due to too large matrices, infinite recursion, or uninitialized variables.\nYou can increase the size of 'Stringbuffer' in dymola/source/matrixop.h.");
	return s;
}
#endif
...
/* Construct array from sizes, using variable number of arguments */
DYMOLA_STATIC RealArray DYNJRealNonTemporary(SizeType ndims, struct DYNFunctionData_*DYNStackData_, ...) {
	RealArray temp;
	va_list ap;
	va_start(ap, DYNStackData_);
...
DYMOLA_STATIC IntegerArray DYNJIntegerNonTemporary(SizeType ndims, struct DYNFunctionData_*DYNStackData_, ...) {
	IntegerArray temp;
	va_list ap;
	va_start(ap, DYNStackData_);
...
DYMOLA_STATIC StringArray DYNJStringNonTemporary(SizeType ndims, struct DYNFunctionData_*DYNStackData_, ...) {
	StringArray temp;
	va_list ap;
	va_start(ap, DYNStackData_);
...
#if defined(DYNCallStackData)
DYMOLA_STATIC IntegerArray DYNJRealSizes(const RealArray a, struct DYNFunctionData_*DYNStackData_) {
	IntegerArray res;
	Integer i;
	res.ndims = 1;
	res.dims = DYNJSizeTemp(1, DYNStackData_);
	res.dims[0] = a.ndims;
	res.data = DYNJIntegerTemp(a.ndims, DYNStackData_);
...
#if defined(DYNCallStackData)
DYMOLA_STATIC RealArray DYNJRealCopy(const RealArray a, struct DYNFunctionData_*DYNStackData_) {
	int i, imax;
	RealArray temp;
	temp.ndims = a.ndims;
	temp.dims = DYNJSizeTemp(a.ndims, DYNStackData_);
	...
	temp.data = DYNJRealTemp(imax, DYNStackData_);
	for (i = 0; i<imax; ++i) temp.data[i] = a.data[i];
	return temp;
}
DYMOLA_STATIC void DYNJRealPutSub(const RealArray ain, struct DYNFunctionData_*DYNStackData_, RealArray out, ...) {
	RealArray a;
	va_list ap;
	SizeType out_dim = 0, a_dim = 0, trailingsize = 1, nargs = 0;
	a = DYNJRealCopy(ain, DYNStackData_);
	/* Pass 1. Determine nr. dimensions of output */
	{
		Integer over = 0;
		va_start(ap, out);
...
#if defined(DYNCallStackData)
DYMOLA_STATIC RealArray DYNJRealvector(const RealArray a, struct DYNFunctionData_*DYNStackData_) {
	RealArray res;
#ifndef NDEBUG
...
DYMOLA_STATIC RealArray DYNJRealmatrix(const RealArray a, struct DYNFunctionData_*DYNStackData_) {
	RealArray res;
	res.ndims = 2;
	res.dims = DYNJSizeTemp(2, DYNStackData_);
	res.dims[0] = a.dims[0];
...
DYMOLA_STATIC RealArray RealsymmetricAssign(RealArray res, const RealArray a) {
	...
	Assert(res.ndims==2 && res.dims[0]==n && res.dims[1]==n, "symmetric requires square matrix");
	for (i = 0; i<n; i++) {
		for (j = 0; j <= i; j++)
...
#if defined(DYNCallStackData)
DYMOLA_STATIC RealArray DYNJRealsymmetric(const RealArray a, struct DYNFunctionData_*DYNStackData_) {
	Integer n = a.dims[0];
	return RealsymmetricAssign(DYNJRealTemporaryMatrix(n, n, DYNStackData_), a);
}
#endif
DYMOLA_STATIC RealArray Realsymmetric(const RealArray a) {
	Integer n = a.dims[0];
	return RealsymmetricAssign(RealTemporaryMatrix(n, n), a);
}
...
DYMOLA_STATIC RealArray RealcrossAssign(RealArray res, const RealArray x, const RealArray y) {
...
DYMOLA_STATIC RealArray RealskewAssign(RealArray res, const RealArray x) {
...
#if defined(DYNCallStackData)
DYMOLA_STATIC RealArray DYNJRealdiagonal(const RealArray a, struct DYNFunctionData_*DYNStackData_) {
	RealArray res;
	Integer i, j, n;
	Assert(a.ndims == 1, "diagonal requires vector input");
	n = a.dims[0];
	res = DYNJRealTemporaryMatrix(n, n, DYNStackData_);
...
DYMOLA_STATIC RealArray DYNJRealcross(const RealArray x, const RealArray y, struct DYNFunctionData_*DYNStackData_) {
	return RealcrossAssign(DYNJRealTemporaryVector(3, DYNStackData_), x, y);
}
DYMOLA_STATIC RealArray DYNJRealskew(const RealArray x, struct DYNFunctionData_*DYNStackData_) {
	return RealskewAssign(DYNJRealTemporaryMatrix(3, 3, DYNStackData_), x);
}
#endif
...
DYMOLA_STATIC RealArray Realcross(const RealArray x,const RealArray y) {
	return RealcrossAssign(RealTemporaryVector(3), x, y);
}
DYMOLA_STATIC RealArray Realskew(const RealArray x) {
	return RealskewAssign(RealTemporaryMatrix(3, 3), x);
}
/* Multiply */
...
DYMOLA_STATIC RealArray RealIdentityAssign(RealArray res, SizeType n) {
	Integer i, j;
	for (i = 0; i<n; i++) for (j = 0; j<n; j++) res.data[i*n + j] = (i == j);
	return res;
}
#if defined(DYNCallStackData)
DYMOLA_STATIC RealArray DYNJRealIdentity(SizeType n, struct DYNFunctionData_*DYNStackData_) {
	Integer i, j;
	return RealIdentityAssign(DYNJRealTemporaryMatrix(n, n, DYNStackData_), n);
}
#endif
DYMOLA_STATIC RealArray RealIdentity(SizeType n) {
	...
	RealIdentityAssign(res, n);
    return res;
}
#if defined(DYNCallStackData)
DYMOLA_STATIC RealArray DYNJRealPow(const RealArray a, const Integer n, struct DYNFunctionData_*DYNStackData_) {
	...
	if (n == 1) return a;
	else if (n == 0) return DYNJRealIdentity(a.dims[0], DYNStackData_);
	else {
		if (n % 2) {
			return DYNJRealMultiplyMM(DYNJRealPow(a, n - 1, DYNStackData_), a, DYNStackData_);
		} else {
			RealArray ahalf = DYNJRealPow(a, n / 2, DYNStackData_);
			return DYNJRealMultiplyMM(ahalf, ahalf, DYNStackData_);
		}
	}
}
#endif
...
#if defined(DYNCallStackData)
DYMOLA_STATIC RealArray DYNJRealRange(const Real from, const Real to, const Real stride, struct DYNFunctionData_*DYNStackData_) {
	...
	if (num<0) num = 0;
	{
		RealArray res = DYNJRealTemporaryVector(num, DYNStackData_);
		Integer i;
...
DYMOLA_STATIC RealArray DYNJlinspace(const Real from, const Real to, const Integer n, struct DYNFunctionData_*DYNStackData_) {
	RealArray res = DYNJRealTemporaryVector(n, DYNStackData_);
	Integer i;
...
#if defined(DYNCallStackData)
DYMOLA_STATIC IntegerArray DYNJIntegerSizes(const IntegerArray a, struct DYNFunctionData_*DYNStackData_) {
	IntegerArray res;
	Integer i;
	res.ndims = 1;
	res.dims = DYNJSizeTemp(1, DYNStackData_);
...
/* The helper in Modelica */
#if defined(DYNCallStackData)
DYMOLA_STATIC IntegerArray DYNJIntegerPromote(const IntegerArray a, SizeType n, struct DYNFunctionData_*DYNStackData_) {
	IntegerArray res;
	SizeType i;
	Assert(n >= a.ndims, "Promote cannot decrease number of dimensions");
	res.ndims = n;
...
#if defined(DYNCallStackData)
/* Matrix operations not limited to numeric matrices */
DYMOLA_STATIC IntegerArray DYNJIntegervector(const IntegerArray a, struct DYNFunctionData_*DYNStackData_) {
	IntegerArray res;
#ifndef NDEBUG
	Integer i, found_non_one = 0;
	for (i = 0; i<a.ndims; i++) { if (a.dims[i]>1) { Assert(!found_non_one, "vector requires exactly one vector non-unit dimension"); found_non_one = 1; } }
...
DYMOLA_STATIC IntegerArray DYNJIntegermatrix(const IntegerArray a, struct DYNFunctionData_*DYNStackData_) {
	IntegerArray res;
	res.ndims = 2;
	res.dims = DYNJSizeTemp(2, DYNStackData_);
	res.dims[0] = a.dims[0];
...
#if defined(DYNCallStackData)
DYMOLA_STATIC IntegerArray DYNJIntegertranspose(const IntegerArray a, struct DYNFunctionData_*DYNStackData_) {
	IntegerArray res;
	Integer i, j, k, remsize;
	res.ndims = a.ndims;
	Assert(a.ndims >= 2, "transpose requires ndims>=2");
...
DYMOLA_STATIC IntegerArray DYNJIntegerouterproduct(const IntegerArray a, const IntegerArray b, struct DYNFunctionData_*DYNStackData_) {
	Assert(a.ndims == 1, "Outerproduct require a vector as first argument");
	Assert(b.ndims == 1, "Outerproduct require a vector as second argument");
	{
		SizeType na = a.dims[0], nb = b.dims[0];
		SizeType i, j;
		IntegerArray res = DYNJIntegerTemporaryMatrix(na, nb, DYNStackData_);
		for (i = 0; i<na; i++) {
			Integer ai = a.data[i];
			for (j = 0; j<nb; j++)
				res.data[i*nb + j] = ai*b.data[j];
...
DYMOLA_STATIC IntegerArray DYNJIntegersymmetric(const IntegerArray a, struct DYNFunctionData_*DYNStackData_) {
	IntegerArray res;
	Integer i, j, n = a.dims[0];
	Assert(a.ndims == 2, "symmetric requires ndims==2");
	Assert(a.dims[0] == a.dims[1], "symmetric requires square matrix");
	res = DYNJIntegerTemporaryMatrix(n, n, DYNStackData_);
	for (i = 0; i<n; i++) {
...
#if defined(DYNCallStackData)
DYMOLA_STATIC IntegerArray DYNJIntegerAdd(const IntegerArray a, const IntegerArray b, struct DYNFunctionData_*DYNStackData_) {
	return IntegerAddAssign(DYNJIntegerMatch(a, DYNStackData_), a, b);
}
#endif
...
#if defined(DYNCallStackData)
DYMOLA_STATIC IntegerArray DYNJIntegerSubtract(const IntegerArray a, const IntegerArray b, struct DYNFunctionData_*DYNStackData_) {
	return IntegerSubtractAssign(DYNJIntegerMatch(a, DYNStackData_), a, b);
}
#endif
...
#if defined(DYNCallStackData)
DYMOLA_STATIC IntegerArray DYNJIntegerScale(const IntegerArray a, const Integer t, struct DYNFunctionData_*DYNStackData_) {
	return IntegerScaleAssign(DYNJIntegerMatch(a, DYNStackData_), a, t);
}
#endif
...
#if defined(DYNCallStackData)
DYMOLA_STATIC IntegerArray DYNJIntegerMinus(const IntegerArray a, struct DYNFunctionData_*DYNStackData_) {
	return IntegerMinusAssign(DYNJIntegerMatch(a, DYNStackData_), a);
}
#endif
...
DYMOLA_STATIC IntegerArray IntegercrossAssign(IntegerArray res, const IntegerArray x, const IntegerArray y) {
	Assert((x.ndims == 1) && (y.ndims == 1), "cross requires vector input");
	Assert(x.dims[0] == 3, "cross requires 3-vector input for first argument");
	Assert(y.dims[0] == 3, "cross requires 3-vector input for second argument");
	res.data[0] = x.data[1] * y.data[2] - x.data[2] * y.data[1];
...
DYMOLA_STATIC IntegerArray IntegerskewAssign(IntegerArray res, const IntegerArray x) {
	Assert(x.ndims == 1, "skew requires vector input");
	Assert(x.dims[0] == 3, "skew requires 3-vector input");
...
#if defined(DYNCallStackData)
DYMOLA_STATIC IntegerArray DYNJIntegerdiagonal(const IntegerArray a, struct DYNFunctionData_*DYNStackData_) {
	IntegerArray res;
	Integer i, j, n;
	Assert(a.ndims == 1, "diagonal requires vector input");
	n = a.dims[0];
...
DYMOLA_STATIC IntegerArray DYNJIntegercross(const IntegerArray x, const IntegerArray y, struct DYNFunctionData_*DYNStackData_) {
	return IntegercrossAssign(DYNJIntegerTemporaryVector(3, DYNStackData_), x, y);
}
DYMOLA_STATIC IntegerArray DYNJIntegerskew(const IntegerArray x, struct DYNFunctionData_*DYNStackData_) {
	return IntegerskewAssign(DYNJIntegerTemporaryMatrix(3, 3, DYNStackData_), x);
}
#endif
...
DYMOLA_STATIC IntegerArray Integercross(const IntegerArray x,const IntegerArray y) {
	return IntegercrossAssign(IntegerTemporaryVector(3), x, y);
}
DYMOLA_STATIC IntegerArray Integerskew(const IntegerArray x) {
	return IntegerskewAssign(IntegerTemporaryMatrix(3, 3), x);
}
/* Multiply */
...
#if defined(DYNCallStackData)
DYMOLA_STATIC IntegerArray DYNJIntegerMultiplyMM(const IntegerArray a, const IntegerArray b, struct DYNFunctionData_*DYNStackData_) {
	return IntegerMultiplyMMAssign(DYNJIntegerTemporaryMatrix(a.dims[0], b.dims[1], DYNStackData_), a, b);
}
DYMOLA_STATIC IntegerArray DYNJIntegerMultiplyMV(const IntegerArray a, const IntegerArray b, struct DYNFunctionData_*DYNStackData_) {
	return IntegerMultiplyMVAssign(DYNJIntegerTemporaryVector(a.dims[0], DYNStackData_), a, b);
}
DYMOLA_STATIC IntegerArray DYNJIntegerMultiplyVM(const IntegerArray a, const IntegerArray b, struct DYNFunctionData_*DYNStackData_) {
	return IntegerMultiplyVMAssign(DYNJIntegerTemporaryVector(b.dims[1], DYNStackData_), a, b);
}
#endif
...
DYMOLA_STATIC IntegerArray IntegerIdentityAssign(IntegerArray res, SizeType n) {
	int i, j;
	for (i = 0; i < n; i++) for (j = 0; j < n; j++) res.data[i*n + j] = (i == j);
	return res;
}
#if defined(DYNCallStackData)
DYMOLA_STATIC IntegerArray DYNJIntegerIdentity(SizeType n, struct DYNFunctionData_*DYNStackData_) {
	return IntegerIdentityAssign(DYNJIntegerTemporaryMatrix(n, n, DYNStackData_), n);
}
#endif
...
#if defined(DYNCallStackData)
DYMOLA_STATIC IntegerArray DYNJIntegerPow(const IntegerArray a, const Integer n, struct DYNFunctionData_*DYNStackData_) {
	Assert(n >= 0, "Matrices can only be raised to positive integer");
	Assert(a.ndims == 2, "Matrix raised to a positive integer requires a matrix as first operand");
	Assert(a.dims[0] == a.dims[1], "Matrix raised to a positive integer requires a square matrix as first operand");
	if (n == 1) return a;
	else if (n == 0) return DYNJIntegerIdentity(a.dims[0], DYNStackData_);
	else {
		if (n % 2) {
			return DYNJIntegerMultiplyMM(DYNJIntegerPow(a, n - 1, DYNStackData_), a, DYNStackData_);
		} else {
			IntegerArray ahalf = DYNJIntegerPow(a, n / 2, DYNStackData_);
			return DYNJIntegerMultiplyMM(ahalf, ahalf, DYNStackData_);
		}
	}
}
#endif
...
/*  Internal: Use an existing va_list */
#if defined(DYNCallStackData)
DYMOLA_STATIC StringArray DYNJStringVaTemporarySize(SizeType ndims, struct DYNFunctionData_*DYNStackData_, va_list ap) {
	StringArray temp;
	SizeType i;
	temp.ndims = ndims;
	temp.dims = DYNJSizeTemp(ndims, DYNStackData_);
...
DYMOLA_STATIC StringArray DYNJStringVaTemporary(SizeType ndims, struct DYNFunctionData_*DYNStackData_, va_list ap) {
	StringArray temp = DYNJStringVaTemporarySize(ndims, DYNStackData_, ap);
	temp.data = DYNJStringTemp(StringNrElements(temp), DYNStackData_);
	return temp;
}
/*  Construct a temporary of the same size as a */
DYMOLA_STATIC StringArray DYNJStringMatch(const StringArray a, struct DYNFunctionData_*DYNStackData_) {
	StringArray temp;
	SizeType i;
	temp.ndims = a.ndims;
	temp.dims = DYNJSizeTemp(a.ndims, DYNStackData_);
	for (i = 0; i<a.ndims; i++) temp.dims[i] = a.dims[i];
	temp.data = DYNJStringTemp(StringNrElements(a), DYNStackData_);
	return temp;
}
/* Construct array from sizes, using variable number of arguments */
DYMOLA_STATIC StringArray DYNJStringTemporary(SizeType ndims, struct DYNFunctionData_*DYNStackData_, ...) {
	StringArray temp;
	va_list ap;
	va_start(ap, DYNStackData_);
	temp = DYNJStringVaTemporary(ndims, DYNStackData_, ap);
	va_end(ap);
	return temp;
}
/* Special cases for vectors and matrices (most often used) */
DYMOLA_STATIC StringArray DYNJStringTemporaryMatrix(SizeType r, SizeType c, struct DYNFunctionData_*DYNStackData_) {
	StringArray temp;
	temp.ndims = 2;
	temp.dims = DYNJSizeTemp(2, DYNStackData_);
	temp.dims[0] = r; temp.dims[1] = c;
	temp.data = DYNJStringTemp(r*c, DYNStackData_);
	return temp;
}
DYMOLA_STATIC StringArray DYNJStringTemporaryVector(SizeType r, struct DYNFunctionData_*DYNStackData_) {
	StringArray temp;
	temp.ndims = 1;
	temp.dims = DYNJSizeTemp(1, DYNStackData_);
	temp.dims[0] = r;
	temp.data = DYNJStringTemp(r, DYNStackData_);
	return temp;
}
#endif
...
#if defined(DYNCallStackData)
DYMOLA_STATIC IntegerArray DYNJStringSizes(const StringArray a, struct DYNFunctionData_*DYNStackData_) {
	IntegerArray res;
	Integer i;
	res.ndims = 1;
	res.dims = DYNJSizeTemp(1, DYNStackData_);
...
#if defined(DYNCallStackData)
DYMOLA_STATIC StringArray DYNJStringArrayArray(SizeType narg, struct DYNFunctionData_*DYNStackData_, StringArray first, ...) {
	StringArray res;
	SizeType i, j, prodsize;
	va_list ap;
	res.ndims = first.ndims + 1;
	...
	res.data = DYNJStringTemp(prodsize*narg, DYNStackData_);
	va_start(ap, first);
	for (i = 0; i<narg; i++) {
		StringArray next;
...
#if defined(DYNCallStackData)
DYMOLA_STATIC StringArray DYNJStringScalarArray(SizeType narg, struct DYNFunctionData_*DYNStackData_, ...) {
	StringArray res;
	SizeType i;
	va_list ap;
	res = DYNJStringTemporaryVector(narg, DYNStackData_);
	va_start(ap, DYNStackData_);
	for (i = 0; i<narg; i++) {
...
#if defined(DYNCallStackData)
DYMOLA_STATIC StringArray DYNJStringCat(SizeType along, struct DYNFunctionData_*DYNStackData_, SizeType nargs, StringArray first, ...) {
	StringArray res;
	va_list ap;
	SizeType i, j;
	res.ndims = first.ndims;
	...
	res.data = DYNJStringTemp(CommonNrElements(res.ndims, res.dims), DYNStackData_);
	{
		/* Pass 2: copy data */
		SizeType presize = 1;      /* Blocks */
		SizeType trailingsize = 1; /* Elements in each block */
...
/* The helper in Modelica */
DYMOLA_STATIC StringArray DYNJStringPromote(const StringArray a, SizeType n, struct DYNFunctionData_*DYNStackData_) {
	StringArray res;
	SizeType i;
	Assert(n >= a.ndims, "Promote cannot decrease number of dimensions");
	res.ndims = n;
...
DYMOLA_STATIC StringArray DYNJStringPromoteScalar(const char* x, SizeType n, struct DYNFunctionData_*DYNStackData_) {
	StringArray res;
	Integer i;
	res.ndims = n;
	res.data = DYNJStringTemp(1, DYNStackData_);
	res.data[0] = x;
	res.dims = DYNJSizeTemp(n, DYNStackData_);
	for (i = 0; i<n; i++) res.dims[i] = 1;
	return res;
...
#if defined(DYNCallStackData)
DYMOLA_STATIC StringArray DYNJStringGetSub(const StringArray a, struct DYNFunctionData_*DYNStackData_, ...) {
	StringArray temp;
	va_list ap;
	SizeType ndims = a.ndims, trailingsize = 1, nargs = 0;
	/* Pass 1. Determine nr. dimensions of output */
	...
	temp.data = DYNJStringTemp(CommonNrElements(temp.ndims, temp.dims), DYNStackData_);
	/* Pass 3. Copy output */
	{
...
#if defined(DYNCallStackData)
DYMOLA_STATIC StringArray DYNJStringCopy(const StringArray a, struct DYNFunctionData_*DYNStackData_) {
	int i, imax;
	StringArray temp;
	temp.ndims = a.ndims;
	temp.dims = DYNJSizeTemp(a.ndims, DYNStackData_);
	...
	temp.data = DYNJStringTemp(imax, DYNStackData_);
	for (i = 0; i<imax; ++i) temp.data[i] = a.data[i];
	return temp;
}
#endif
...
#if defined(DYNCallStackData)
DYMOLA_STATIC StringArray DYNJStringFill(const char* t, struct DYNFunctionData_*DYNStackData_, SizeType ndims, ...) {
	va_list ap;
	StringArray temp;
	va_start(ap, ndims);
	temp = StringFillAssign(DYNJStringVaTemporary(ndims, DYNStackData_, ap), t);
	va_end(ap);
	return temp;
}
#endif
...
#if defined(DYNCallStackData)
DYMOLA_STATIC StringArray DYNJStringFillArray(const StringArray a, struct DYNFunctionData_*DYNStackData_, SizeType ndims, ...) {
	va_list ap;
	int i, j, inputProd, fillProd;
	StringArray temp;
	temp.ndims = a.ndims + ndims;
	...
	temp.data = DYNJStringTemp(inputProd*fillProd, DYNStackData_);
	for (i = 0; i<inputProd; ++i) {
		for (j = 0; j<fillProd; j++) {
			temp.data[i + j*inputProd] = a.data[i];
		}
...
#if defined(DYNCallStackData)
DYMOLA_STATIC StringArray DYNJStringvector(const StringArray a, struct DYNFunctionData_*DYNStackData_) {
	StringArray res;
#ifndef NDEBUG
	Integer i, found_non_one = 0;
	for (i = 0; i<a.ndims; i++) { if (a.dims[i]>1) { Assert(!found_non_one, "vector requires exactly one vector non-unit dimension"); found_non_one = 1; } }
...
DYMOLA_STATIC StringArray DYNJStringmatrix(const StringArray a, struct DYNFunctionData_*DYNStackData_) {
	StringArray res;
	res.ndims = 2;
	res.dims = DYNJSizeTemp(2, DYNStackData_);
	res.dims[0] = a.dims[0];
...
#if defined(DYNCallStackData)
DYMOLA_STATIC StringArray DYNJStringtranspose(const StringArray a, struct DYNFunctionData_*DYNStackData_) {
	StringArray res;
	Integer i, j, k, remsize;
	res.ndims = a.ndims;
	Assert(a.ndims >= 2, "transpose requires ndims>=2");
...
DYMOLA_STATIC StringArray DYNJStringsymmetric(const StringArray a, struct DYNFunctionData_*DYNStackData_) {
	StringArray res;
	Integer i, j, n = a.dims[0];
	Assert(a.ndims == 2, "symmetric requires ndims==2");
	Assert(a.dims[0] == a.dims[1], "symmetric requires square matrix");
	res = DYNJStringTemporaryMatrix(n, n, DYNStackData_);
	for (i = 1; i <= n; i++) {
...
#if defined(DYNCallStackData)
DYMOLA_STATIC RealArray DYNJRealSwitchMajor(const RealArray x, struct DYNFunctionData_*DYNStackData_) {
	if (x.ndims <= 1)
		return x;
	{
		RealArray temp = DYNJRealMatch(x, DYNStackData_);
...
#if defined(DYNCallStackData)
DYMOLA_STATIC IntegerArray DYNJIntegerSwitchMajor(const IntegerArray x, struct DYNFunctionData_*DYNStackData_) {
	if (x.ndims <= 1)
		return x;
	{IntegerArray temp = DYNJIntegerMatch(x, DYNStackData_);
	{int i; for (i = 0; i<x.ndims; i++) temp.dims[i] = x.dims[x.ndims - i - 1]; /* Switch dimensions */}
...
#if defined(DYNCallStackData)
DYMOLA_STATIC StringArray DYNJStringSwitchMajor(const StringArray x, struct DYNFunctionData_*DYNStackData_) {
	if (x.ndims <= 1)
		return x;
	{StringArray temp = DYNJStringMatch(x, DYNStackData_);
	{int i; for (i = 0; i<x.ndims; i++) temp.dims[i] = x.dims[x.ndims - i - 1]; /* Switch dimensions */}
...
#include "sprwat.h"
#endif
DYMOLA_STATIC RealArray readMatrixAssign(RealArray m,const char*fil,const char*matnam,Integer rows,Integer cols) {
#if !defined(DYMOLA_DSPACE) && !defined(NO_FILE)
	Amatrix amatrix;
	AmatGetFile afile;
...
#if defined(DYNCallStackData)
DYMOLA_STATIC RealArray DYNJreadMatrix(const char*fil, const char*matnam, Integer rows, Integer cols, struct DYNFunctionData_*DYNStackData_) {
	return readMatrixAssign(DYNJRealTemporaryMatrix(rows, cols, DYNStackData_), fil, matnam, rows, cols);
}
#endif
DYMOLA_STATIC RealArray readMatrix(const char*fil, const char*matnam, Integer rows, Integer cols) {
	return readMatrixAssign(RealTemporaryMatrix(rows, cols), fil, matnam, rows, cols);
}

DYMOLA_STATIC IntegerArray readMatrixSizeAssign(IntegerArray m, const char*fil,const char*matnam) {
#if !defined(DYMOLA_DSPACE) && !defined(NO_FILE)
	Amatrix amatrix;
	AmatGetFile afile;
	int ret=amatGetOpen((char*)fil,"NoClass",(char*)0,&afile);
...
#if defined(DYNCallStackData)
DYMOLA_STATIC IntegerArray DYNJreadMatrixSize(const char*fil, const char*matnam, struct DYNFunctionData_*DYNStackData_) {
	return readMatrixSizeAssign(DYNJIntegerTemporaryVector(2, DYNStackData_), fil, matnam);
}
#endif
DYMOLA_STATIC IntegerArray readMatrixSize(const char*fil, const char*matnam) {
	return readMatrixSizeAssign(IntegerTemporaryVector(2), fil, matnam);
}
#if defined(DYNCallStackData)
DYMOLA_STATIC StringArray DYNJreadMATmatrices_M(const char*fil, struct DYNFunctionData_*DYNStackData_) {
	StringArray s;
#if !defined(DYMOLA_DSPACE) && !defined(NO_FILE)
	...
	amatGetClose(&afile);
	s = StringFillAssign(DYNJStringTemporaryMatrix(nrMatrices, 2, DYNStackData_), "");
...
#else
	Assert(false, "Cannot read files");
	s = DYNJStringTemporaryVector(0, DYNStackData_);
#endif
	return s;
}
#endif
...
#if defined(DYNCallStackData)
DYMOLA_STATIC RealArray DYNJRealReadArray(const char*fil, const char*matnam, Integer ndims, struct DYNFunctionData_*DYNStackData_) {
	RealArray m;
#if !defined(DYMOLA_DSPACE) && !defined(NO_FILE)
	Amatrix amatrix;
	AmatGetFile afile;
	...
	m.ndims = ndims;
	m.dims = DYNJSizeTemp(ndims, DYNStackData_);
	ret = amatGetOpen((char*)fil, "NoClass", (char*)0, &afile);
	found = 0;
	foundSize = 0;
	if (ndims > 2) {
	...
	for (; ret == 0 && !found;) {
		...
		if (ret <= 1 && strcmp(matnam, amatrix.name) == 0) {
			...
			Assert(amatrix.type == doubleMatrix || amatrix.type == realMatrix || amatrix.type == integerMatrix, "No string matrices allowed");
			found = 1;
			m.data = DYNJRealTemp(rows*cols, DYNStackData_);
			if (!foundSize) {
	...
	amatGetClose(&afile);
	Assert(found, "Read of matrix failed");
	if (ndims == 1) return DYNJRealvector(m, DYNStackData_);
#else
	Assert(false, "Cannot read files");
	m = DYNJRealTemporaryMatrix(0, 0, DYNStackData_);
#endif
	return m;
}
DYMOLA_STATIC Real DYNJRealReadScalar(const char*fil, const char*matnam, struct DYNFunctionData_*DYNStackData_) {
	/*
	...
	m = DYNJRealTemporaryMatrix(0, 0, DYNStackData_);
#else
	m = DYNJRealReadArray(fil, matnam, 2, DYNStackData_);
#endif
	val = Realscalar(m);
	return val;
}
...
DYMOLA_STATIC RealArray RealReadArray(const char*fil,const char*matnam,Integer ndims) {
	...
	for(;ret==0 && !found;) {
		...
		Assert(ret<=1,amatError);
		if (ret<=1 && strcmp(matnam,amatrix.name)==0) {
			int i,rows=amatrix.nrow,cols=amatrix.ncol;
			Assert(amatrix.type==doubleMatrix || amatrix.type==realMatrix || amatrix.type==integerMatrix,"No string matrices allowed");
			found=1;
...
#if defined(DYNCallStackData)
DYMOLA_STATIC IntegerArray DYNJIntegerReadArray(const char*fil, const char*matnam, Integer ndims, struct DYNFunctionData_*DYNStackData_) {
	IntegerArray m;
#if !defined(DYMOLA_DSPACE) && !defined(NO_FILE)
	Amatrix amatrix;
	AmatGetFile afile;
	...
	for (; ret == 0 && !found;) {
		...
		if (ret <= 1 && strcmp(matnam, amatrix.name) == 0) {
			...
			Assert(amatrix.type == integerMatrix, "Integer matrix required");
			found = 1;
			m.data = DYNJIntegerTemp(rows*cols, DYNStackData_);
			if (!foundSize) {
				m.dims[0] = amatrix.nrow;
				m.dims[1] = amatrix.ncol;
			}
	...
	amatGetClose(&afile);
	Assert(found, "Read of matrix failed");
	if (ndims == 1) return DYNJIntegervector(m, DYNStackData_);
#else
	Assert(false, "Cannot read files");
	m = DYNJIntegerTemporaryMatrix(0, 0, DYNStackData_);
#endif
	return m;
}
DYMOLA_STATIC Integer DYNJIntegerReadScalar(const char*fil, const char*matnam, struct DYNFunctionData_*DYNStackData_) {
	/*
	Modified 2008-06-11 Dan Henriksson
	Original implementation caused fatal error in
	...
	m = DYNJIntegerTemporaryMatrix(0, 0, DYNStackData_);
#else
	m = DYNJIntegerReadArray(fil, matnam, 2, DYNStackData_);
#endif
	val = Integerscalar(m);
	return val;
}
DYMOLA_STATIC StringArray DYNJStringReadArray(const char*fil, const char*matnam, Integer ndims, struct DYNFunctionData_*DYNStackData_) {
	StringArray m;
#if !defined(DYMOLA_DSPACE) && !defined(NO_FILE)
	Amatrix amatrix;
	AmatGetFile afile;
	...
	for (; ret == 0 && !found;) {
		...
		if (ret <= 1 && strcmp(matnam, amatrix.name) == 0) {
			...
			found = 1;
			m.data = DYNJStringTemp(rows, DYNStackData_);
			if (!foundSize) {
				m.dims[0] = amatrix.nrow;
			}
			for (i = 0; i < rows; ++i) {
	...
	amatGetClose(&afile);
	Assert(found, "Read of matrix failed");
	if (ndims == 1) return DYNJStringvector(m, DYNStackData_);
#else
	Assert(false, "Cannot read files");
	m = DYNJStringTemporaryVector(0, DYNStackData_);
#endif
	return m;
}
DYMOLA_STATIC String DYNJStringReadScalar(const char*fil, const char*matnam, struct DYNFunctionData_*DYNStackData_) {
	/*
	Modified 2008-06-11 Dan Henriksson
	Original implementation caused fatal error in
	VS 8 compiler for HILS on xPC */
	...
	m = DYNJStringTemporaryVector(0, DYNStackData_);
#else
	m = DYNJStringReadArray(fil, matnam, 1, DYNStackData_);
#endif
	val = Stringscalar(m);
	return val;
}
...
DYMOLA_STATIC void DYNJPopMarkMarks(struct DYNFunctionData_*DYNStackData_) {
	...
	*(DYNStackData_->m_stringmark) = (DYNStackData_->m_extra.m_startstringmark);
}
#endif
...
#if defined(DYNCallStackData)
DYMOLA_STATIC char* DYNJGetStringMark(struct DYNFunctionData_*DYNStackData_) {
	return (char*)(*(DYNStackData_->m_stringmark));
}
#endif
...
DYMOLA_STATIC void DYNJSetStringMark(char*s, struct DYNFunctionData_*DYNStackData_) {
	if (s) *(DYNStackData_->m_stringmark) = s;
}
#endif
...
#if defined(DYNCallStackData)
DYMOLA_STATIC const char* DYNJSqueezeString(const char*s, char*startMarker, struct DYNFunctionData_*DYNStackData_) {
	if (s >= startMarker && s<*(DYNStackData_->m_stringmark)) {
		int len = strlen(s) + 1;
		int i;
		if (s + len <= *(DYNStackData_->m_stringmark)) {
			*(DYNStackData_->m_stringmark) = startMarker + len;
			for (i = 0; i<len; ++i) startMarker[i] = s[i]; /* Might overlap*/
			s = startMarker;
		}
	} else {
		*(DYNStackData_->m_stringmark) = startMarker;
	}
	return s;
}
#endif
...
DYMOLA_STATIC StringArray DYNJStringTemporaryDense(String*d, struct DYNFunctionData_*DYNStackData_, SizeType ndims, ...) {
	StringArray temp;
	int i, nrElem;
	va_list ap;
	va_start(ap, ndims);
	temp = DYNJStringVaTemporarySize(ndims, DYNStackData_, ap);
	va_end(ap);
	temp.data = d;
	return temp;
}
DYMOLA_STATIC IntegerArray DYNJIntegerTemporaryDenseOrig(Integer*d, struct DYNFunctionData_*DYNStackData_, SizeType ndims, ...) {
	IntegerArray temp;
	va_list ap;
	va_start(ap, ndims);
	temp = DYNJIntegerVaTemporarySize(ndims, DYNStackData_, ap);
	va_end(ap);
	temp.data = d;
	return temp;
}
...
DYMOLA_STATIC IntegerArray ANDArrayAssign(IntegerArray res, const IntegerArray a,const IntegerArray b) {
	SizeType prodsize,i;
...
#if defined(DYNCallStackData)
DYMOLA_STATIC IntegerArray DYNJANDArray(const IntegerArray a, const IntegerArray b, struct DYNFunctionData_*DYNStackData_) {
	return ANDArrayAssign(DYNJIntegerMatch(a, DYNStackData_), a, b);
}
#endif
DYMOLA_STATIC IntegerArray ANDArray(const IntegerArray a, const IntegerArray b) {
	return ANDArrayAssign(IntegerMatch(a), a, b);
}
DYMOLA_STATIC IntegerArray ORArrayAssign(IntegerArray res, const IntegerArray a,const IntegerArray b) {
	SizeType prodsize,i;
...
#if defined(DYNCallStackData)
DYMOLA_STATIC IntegerArray DYNJORArray(const IntegerArray a, const IntegerArray b, struct DYNFunctionData_*DYNStackData_) {
	return ORArrayAssign(DYNJIntegerMatch(a, DYNStackData_), a, b);
}
#endif
DYMOLA_STATIC IntegerArray ORArray(const IntegerArray a, const IntegerArray b) {
	return ORArrayAssign(IntegerMatch(a), a, b);
}
DYMOLA_STATIC IntegerArray NOTArrayAssign(IntegerArray res, const IntegerArray a) {
	SizeType prodsize,i;
	prodsize=IntegerNrElements(a);
	for(i=0;i<prodsize;i++) res.data[i]=!a.data[i];
	return res;
...
#if defined(DYNCallStackData)
DYMOLA_STATIC IntegerArray DYNJNOTArray(const IntegerArray a, struct DYNFunctionData_*DYNStackData_) {
	return NOTArrayAssign(DYNJIntegerMatch(a, DYNStackData_), a);
}
#endif
DYMOLA_STATIC IntegerArray NOTArray(const IntegerArray a) {
	return NOTArrayAssign(IntegerMatch(a), a);
}
