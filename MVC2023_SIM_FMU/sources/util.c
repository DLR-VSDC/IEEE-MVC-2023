							   ...
							   FMIString category, FMIString message, ...)
{
	...
	if (category == loggFuncCall || comp->loggingOn == FMIFalse && (status < FMIWarning)) {
		return;
	}
...
/* #ifdef _DEBUG fails on dSPACE platforms where _DEBUG is always defined, 0 or 1. */
	...
	if (comp->mStatus != modelInstantiated) {
		util_logger(comp, comp->instanceName, FMIWarning, loggBadCall, "model cannot be initialized in current state(%d)", comp->mStatus);
		return FMIWarning;
	}
...
DYMOLA_STATIC FMIStatus util_exit_model_initialization_mode(FMIComponent c, const char* label, ModelStatus nextStatus)
{
	...
	if (comp->mStatus != modelInitializationMode) {
		util_logger(comp, comp->instanceName, FMIWarning, loggBadCall, "%s: may only called in initialization mode", label);
		return FMIWarning;
	}
