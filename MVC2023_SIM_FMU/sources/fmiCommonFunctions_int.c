...
#define ALLOC_AND_CHECK2(dest, n, type)    	     					\
	dest = (type*) allocateMemory(n, sizeof(type));	                \
	if (dest == NULL) {												\
		goto mem_fail;                                              \
	} else {														\
		freeList[freeIndex++] = dest;								\
...
#if (defined(FMI_2) &&  !(defined(NO_FILE) || defined(RTTWINCAT)) && !defined(BUILD_LIBFMI)) || defined(FMU_SOURCE_CODE_EXPORT_SPARSE)
/*Needed for tpl and superLU aslo declared in libds so only delcare if source code*/
...
DYMOLA_STATIC FMIStatus fmiFreeFMUstate_(FMIComponent c, FMIFMUstate* FMUstate)
{
	...
	if (comp->isCoSim && !comp->istruct->mInlineIntegration) {
		if(integration_free_state(c, target) == integrationFatal)
			return FMIFatal;
	}
#endif /* ONLY_INCLUDE_INLINE_INTEGRATION */
	freeMemory(*FMUstate);
...
DYMOLA_STATIC FMIStatus fmiGetFMUstate_(FMIComponent c, FMIFMUstate* FMUstate)
{
	...
	if(source->storeResult){
		util_logger(source, source->instanceName, FMIWarning, loggBadCall,"%s failed, FMU uses internal result generation; this is not possible to combine with %s",label,label); 
		return FMIWarning;
	}
...
DYMOLA_STATIC FMIStatus fmiSetFMUstate_(FMIComponent c, FMIFMUstate FMUstate)
{
	...
	if(target->storeResult){
		util_logger(target, target->instanceName, FMIWarning, loggBadCall,"%s failed, FMU uses internal result generation; this is not possible to combine with %s",label,label); 
		return FMIWarning;
	}
	if (FMUstate == NULL) {
		util_logger(target, target->instanceName, FMIWarning, loggBadCall, "%s: FMUstate == NULL",label);
		return FMIWarning;
	}
												...
												FMIFMUstate* FMUstate)
{
	...
	EXCEPTION_TRY
		...
		if (tpl_load(tn, TPL_MEM, serializedState, size) != 0) {
			util_logger(comp, comp->instanceName, FMIWarning, loggUndef, "%s: tpl_load failed",label);
			status = FMIWarning;
		} else {
			/* restore state */
			size_t j;
			...
			if (tpl_unpack(tn, 0) < 0) {
				util_logger(comp, comp->instanceName, FMIWarning, loggUndef, "%s: tpl_unpack failed", label);
				status = FMIError;
				goto clean;
			} else {
				/* restore did */
			...
			for (j = 0; j < state->nSPar; j++) {
				...
				if (sParameter)
				{
					state->sParameters[j] = (FMIChar*) comp->functions->allocateMemory(MAX_STRING_SIZE + 1, sizeof(FMIChar));
...
clean2:
	...
	if(status == FMIError)
		return util_error(comp);
#else
	util_logger(comp, comp->instanceName, FMIWarning, loggUndef, "%s: FMU is compiled with flag NO_FILE indicating that the running system does not have a file system,\r\n use fmi2Get/SetFMUstate instead",label);
	status = FMIWarning;
#endif
	return status;
}
...
#define staticListSize 128
	...
	if(source->nSPar>100){
		freeList = (void**) allocateMemory(source->nSPar+nbrStaticAllocs,sizeof(void*));
		if(NULL == freeList){
			return util_error(source);
		}
		freeListDynamic = 1;
	...
	for (i = 0; i < target->nSPar; ++i) {
		ALLOC_AND_CHECK2(target->sParameters[i], MAX_STRING_SIZE + 1, FMIChar);
	}
...
#endif
	if(freeListDynamic){
		source->functions->freeMemory(freeList);
		freeList=0;
		freeListDynamic = 0;
	}
...
static tpl_node* createTplMap(Component* comp, tpl_bin *tplDidBin)
{
	...
	if (tn == NULL) {
		util_logger(comp, comp->instanceName, FMIWarning, loggUndef, "createTplMap: tpl_map returned NULL");
	}
