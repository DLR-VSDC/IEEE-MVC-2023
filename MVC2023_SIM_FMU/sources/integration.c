...
static int check_step_event(void* user_data) {
	...
	return (comp->istruct->mTriggerStepEvent != 0) || steady_state_reached(comp);
}
...
/* ------------------------------------------------------ */
static int steady_state_reached(Component* comp) {
	if (GetAdditionalFlags(9) && comp && comp->iData) {
		IntegrationData* iData = comp->iData;
		...
		if (abstol_data && reltol_data) {
			...
			const double tolerance = (tolerance_flag > 0 ? tolerance_flag : (2.0e-2 / (comp->tStop - comp->tStart)));
			const double quotient = tolerance / (comp->relativeTolerance > 0.0 ? comp->relativeTolerance : 0.0001) / 2.0;
			unsigned i;
			for (i = 0; i < comp->nStates; ++i) {
